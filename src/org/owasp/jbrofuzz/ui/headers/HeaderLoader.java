...
public class HeaderLoader {
	...
	public Header getHeader(final TreeNode [] Id) {
		...
		for(String[] k : set) {
			...
			int success = 1;
			if(Id.length == k.length + 1) {	
				// Compare every element within the array, ignoring the first
				for(int i = 1; i < k.length; i++) {
					
					if(Id[i].toString().equals(k[i-1].toString())) {
						success++;
					}
				}
			}
		...
		// System.out.println("Returning Header 0");
		return new Header(0);
